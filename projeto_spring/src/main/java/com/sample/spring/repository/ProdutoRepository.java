...
public interface ProdutoRepository extends JpaRepository<Produto, Long> {
	...
	@Query("select p from Produto p left join fetch p.tipoproduto t where UPPER(t.name) like UPPER(?1) or UPPER(t.pagamento) like UPPER(?1) or UPPER(p.pagamento) like UPPER(?1)")
